  ...
  while more_pages:
    ...
    if 'entries' in page:
      for ad_group in page['entries']:
        curr = response[str(row.ad_group.id.value)]
        curr['ad_group_name'] = row.ad_group.name.value
        curr['ad_group_status'] = curr_status
        curr['ad_group_id'] = row.ad_group.id.value
